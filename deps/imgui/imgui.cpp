...
void ImStrncpy(char* dst, const char* src, size_t count)
{
    if (count < 1)
        return;
    if (count > 1)
        strncpy_s(dst, sizeof(dst), src, count - 1);
    dst[count - 1] = 0;
}
...
FILE* ImFileOpen(const char* filename, const char* mode)
{
    ...
    FILE *ptr;
    _wfopen_s(&ptr, (wchar_t*)&buf[0], (wchar_t*)&buf[filename_wsize]);
    return ptr;
#else
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    if ((g.NavMoveDir == ImGuiDir_Down) || (g.NavMoveDir == ImGuiDir_Up))
    {
        // ###########
        // START PARSEC ONLY HACK
        // ###########

        if (curr.Min.x > 57 && cand.Min.x <= 57) return false;

        // ###########
        // END PARSEC ONLY HACK
        // ###########
        distance_od = (cand.Min.x + cand.Max.x) - (curr.Min.x + curr.Max.x);
        distance = (cand.Min.y + cand.Max.y) - (curr.Min.y + curr.Max.y);

        if ((g.NavMoveDir == ImGuiDir_Down) && ((distance <= 0.0f) || (curr.Max.y > cand.Max.y)))
        {
            return false;
        }
        else if ((g.NavMoveDir == ImGuiDir_Up) && ((distance >= 0.0f) || (curr.Max.y < cand.Max.y)))
        {
            return false;
        }
    }
    ...
    {
        distance_od = (cand.Min.y + cand.Max.y) - (curr.Min.y + curr.Max.y);
        distance = (cand.Min.x + cand.Max.x) - (curr.Min.x + curr.Max.x);

        if ((g.NavMoveDir == ImGuiDir_Left) && ((distance >= 0.0f) || (curr.Max.x < cand.Max.x)))
        {
            return false;
        }
        else if ((g.NavMoveDir == ImGuiDir_Right) && ((distance <= 0.0f) || (curr.Max.x > cand.Max.x)))
        {
            return false;
        }
    }
#if IMGUI_DEBUG_NAV_SCORING
    printf("drawCurrent(ctx, %f, %f, %f, %f);\n", curr.Min.x, curr.Max.x, curr.Min.y, curr.Max.y);
    printf("drawCandidate(ctx, %f, %f, %f, %f);\n", cand.Min.x, cand.Max.x, cand.Min.y, cand.Max.y);
    printf("Dist od: %f %f\n", distance, distance_od);
#endif
    ...
    if ((distance_od == 0) && (!result->Direct || distance < result->DistBox))
    {
        result->DistBox = distance;
        result->Direct = true;
        return true;
    }
    ...
    {
        float Score = distance + (distance_od * 2.0f);
        if (Score < result->DistBox)
        {
            result->DistBox = Score;
            return true;
        }
    }
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb, const ImGuiID id)
{
    ...
    {
        ImGuiNavMoveResult* result = ((window->Flags | g.NavWindow->Flags) & ImGuiWindowFlags_NavFlattened) ?
...
#else
    const bool home_pressed = false;
    const bool end_pressed = false;
#endif
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    int x, y;
    int i;
    if (sscanf_s(line, "Pos=%i,%i", &x, &y) == 2)         settings->Pos = ImVec2ih((short)x, (short)y);
    else if (sscanf_s(line, "Size=%i,%i", &x, &y) == 2)   settings->Size = ImVec2ih((short)x, (short)y);
